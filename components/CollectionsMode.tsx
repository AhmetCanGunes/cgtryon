...
import { Upload, Sparkles, Loader2, LayoutTemplate, RefreshCw, Download, ChevronLeft, ChevronRight, Check, Images, Wand2, Calendar, CloudSun } from 'lucide-react';
import {
    CollectionSettings,
    AD_PLATFORMS,
    AD_THEMES,
...
import { generateSimilarCollectionImage } from '../services/geminiService';

interface CollectionsModeProps {
  onGenerate: (image: File, settings: CollectionSettings) => Promise<CollectionResult[]>;
...
const CollectionsMode: React.FC<CollectionsModeProps> = ({ onGenerate, isGenerating }) => {
  ...
  const [settings, setSettings] = useState<CollectionSettings>({
    ...
    pose: MODEL_POSE_OPTIONS[0],
    numberOfImages: 1,
    specificVariationId: 'auto',
  ...
  const handleGenerateSimilar = async (count: number) => {
      ...
      const referenceResult = resultImages[selectedIndex];
      setShowSimilarCount(false);
      setIsGeneratingSimilar(true);

      try {
          const newResults = await generateSimilarCollectionImage(productImage, referenceResult.metadata, count);
      ...
      <div className="w-[380px] flex-shrink-0 border-r border-white/10 bg-bg-surface flex flex-col h-full z-20">
        <div className="flex-1 overflow-y-auto custom-scrollbar p-3 flex flex-col gap-2">
            ...
            <div className="flex flex-col gap-1">
                <span className="text-[9px] font-bold text-gray-500 uppercase tracking-wider">Model / Manken Görseli</span>
                <div
                ...
                >
                    {productImage ? (
                        <>
                            <img src={URL.createObjectURL(productImage)} className="w-full h-full object-contain p-1" alt="Model" />
            ...
            <div className="space-y-2">
                ...
                {/* Pose (full width, no font dropdown) */}
                <div className="flex flex-col gap-1">
        ...
        {resultImages.length > 0 && resultImages[selectedIndex] ? (
            <div className="flex flex-col items-center w-full h-full max-h-full">
                ...
                <div className="relative flex-1 w-full flex items-center justify-center min-h-0 mb-4 group">
                     <img
                        ...
                        alt={`Collection Variant ${selectedIndex + 1}`}
            ...
            <div className="flex flex-col items-center justify-center text-gray-500 select-none pointer-events-none">
                ...
                <p className="text-xs text-gray-500 max-w-xs text-center">
                    Model görseli yükleyin ve "Görsel Oluştur" butonuna basın.
                </p>
            </div>
        )}
      </div>
