...
import {
  WomensFashionSettings,
  WOMENS_FASHION_PURPOSE_OPTIONS,
  WOMENS_FASHION_POSE_OPTIONS,
  WOMENS_FASHION_BACKGROUNDS,
  WOMENS_FASHION_BACKGROUND_STYLES,
  SKIN_TONE_OPTIONS,
  ...
  // Yeni eklenen seçenekler (Erkek Giyimden)
  COLLAR_TYPE_OPTIONS,
  FABRIC_TYPE_OPTIONS,
...
import {
  generateWomensFashionImage,
  generateMultipleWomensFashionImages,
  ...
  buildWomensFashionPromptPreview,
...
interface WomensFashionModeProps {
  onClose?: () => void;
  userCredits?: number;
  isUserAdmin?: boolean;
  onShowPricing?: () => void;
...
// Dress icon component
const DressIcon = ({ className }: { className?: string }) => (
  <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M6 3h12l-3 9h-6L6 3z" />
    <path d="M9 12v9" />
    <path d="M15 12v9" />
    <path d="M6 21h12" />
    <path d="M9 12c0 0-3 3-3 6s3 3 3 3" />
    <path d="M15 12c0 0 3 3 3 6s-3 3-3 3" />
  </svg>
);

const WomensFashionMode: React.FC<WomensFashionModeProps> = ({
  onClose,
  userCredits = 0,
  isUserAdmin = false,
  onShowPricing,
  ...
  const [results, setResults] = useState<WomensFashionGenerationResult[]>([]);
  ...
  // Preset (Hazır Sahne) state'leri
  const [presetClothes, setPresetClothes] = useState<File[]>([]);
  ...
  // Preset kategori filtresi
  const [presetCategoryFilter, setPresetCategoryFilter] = useState<string>('all');

  const [settings, setSettings] = useState<WomensFashionSettings>({
    purpose: 'ecommerce',
    skinTone: 'medium',
    poseStyle: 'neutral',
    background: 'white',
    ...
    // Erkek Giyimden eklenen default değerler
    floorType: 'none',
    lightingStyle: 'natural',
    woodType: 'oak',
    aspectRatio: '1:1',
    cameraFrame: 'full_body',
    ...
    fabricType: 'cotton',
    spallaCamicia: 'auto',
    pleatType: 'flat_front',
    trouserFit: 'regular',
    studioMinimalColor: '#f5f5f5',
  ...
  const updateSetting = <K extends keyof WomensFashionSettings>(key: K, value: WomensFashionSettings[K]) => {
  ...
  // Referanslı üretim hayvan ayarlarını güncelle
  ...
  // Sahneye yerleştir hayvan ayarlarını güncelle
  ...
  const handleShowPromptPreview = () => {
    const prompt = buildWomensFashionPromptPreview(
      settings,
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (settings.numberOfImages === 1) {
        const result = await generateWomensFashionImage(sourceImage, settings, (msg) => setProgress(msg), refModel, additionalClothes);
        setResults([result]);
        // Başarılı üretim sonrası kredi düş
        if (!isUserAdmin) onCreditsUsed?.(1);
      } else {
        const results = await generateMultipleWomensFashionImages(sourceImage, settings, (msg, current, total) => {
          setProgress(msg);
  ...
  const handleDownload = (imageData: string, index: number) => {
    ...
    link.href = imageData;
    link.download = `womens-fashion-${settings.purpose}-${index + 1}.png`;
  ...
  const handleRefDownload = (imageData: string, index: number) => {
    const link = document.createElement('a');
    link.href = imageData;
    link.download = `referenced-production-${index + 1}.png`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };
  ...
  const handleRef2Download = (imageData: string, index: number) => {
    const link = document.createElement('a');
    link.href = imageData;
    link.download = `womens-referenced2-production-${index + 1}.png`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };
  ...
  const handleSceneDownload = (imageData: string, index: number) => {
    const link = document.createElement('a');
    link.href = imageData;
    link.download = `scene-placement-${index + 1}.png`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };
  ...
  const handlePresetClothesUpload = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      const newFiles = Array.from(files).slice(0, 7 - presetClothes.length) as File[];
      setPresetClothes(prev => [...prev, ...newFiles]);
  ...
  const handleShowPresetPromptPreview = () => {
    const prompt = buildPresetProductionPromptPreview('custom', presetSettings, presetClothes.length, presetUseCustomModel);
    setCurrentPromptPreview(prompt);
    setShowPromptPreview(true);
  };

  const handlePresetGenerate = async () => {
    if (presetClothes.length === 0 || !customBackground) return;
    ...
    try {
      ...
      if (numberOfImages === 1 && !presetSettings.useAngleSet) {
        ...
        const results = await generateMultiplePresetProductions(
          ...
          (msg: string, current: number, total: number) => {
            setProgress(`Görsel ${current}/${total} oluşturuluyor... ${msg}`);
            setProgressCount({ current, total });
          },
          presetUseCustomModel ? presetModel : null,
          customBackground
      ...
      setError(err instanceof Error ? err.message : 'Görsel oluşturulurken bir hata oluştu');
    } finally {
      setIsProcessing(false);
      setProgress('');
      setProgressCount(null);
  ...
  const handlePresetDownload = (imageData: string, index: number) => {
    const link = document.createElement('a');
    link.href = imageData;
    link.download = `womens-preset-${index + 1}.png`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };
  ...
  return (
    <>
      ...
      <aside className="w-[380px] flex-shrink-0 bg-bg-surface border-r border-white/10 flex flex-col h-full">
        ...
        <input type="file" ref={presetClothesInputRef} onChange={handlePresetClothesUpload} accept="image/*" multiple className="hidden" />
          ...
          {productionMode === 'standard' ? (
            ...
            <div className="bg-white/5 rounded-lg p-2 border border-white/10">
              ...
              {(additionalClothes.length > 0 || referenceModelPreview) && (
                <p className="text-[9px] text-gray-500 mt-2">
                  ...
                  {additionalClothes.length > 0 && referenceModelPreview && ' • '}
                  {referenceModelPreview && 'Referans model aktif'}
                </p>
              )}
            </div>
            ...
            <div className="bg-white/5 rounded-lg p-2 border border-white/10">
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-gray-900 text-xs font-medium">Çekim Ölçeği</h3>
                <button
                  onClick={() => {
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full appearance-none bg-white border border-gray-200 rounded-lg px-3 py-2 pr-8 text-xs text-gray-700 focus:outline-none focus:ring-2 focus:ring-mode-accent focus:border-transparent cursor-pointer hover:border-border-strong"
                >
            ...
            {/* Kamera Açısı */}
            <div className="bg-white/5 rounded-lg p-2 border border-white/10">
              <h3 className="text-gray-900 text-xs font-medium mb-2">Kamera Açısı</h3>
              <div className="relative">
                <select
            ...
            {/* Lens */}
            <div className="bg-white/5 rounded-lg p-2 border border-white/10">
              <h3 className="text-gray-900 text-xs font-medium mb-2">Lens</h3>
              <div className="relative">
                <select
            ...
            <div className="bg-white/5 rounded-lg p-2 border border-white/10">
              <h3 className="text-gray-900 text-xs font-medium mb-2">Işık Stili</h3>
            ...
            {/* Özel Prompt Girişi */}
            <div className="bg-white/5 rounded-lg p-2 border border-white/10">
              <h3 className="text-gray-900 text-xs font-medium mb-2">✏️ Özel Talimat (Opsiyonel)</h3>
              <textarea
                value={settings.customPrompt || ''}
                onChange={(e) => updateSetting('customPrompt', e.target.value)}
                placeholder="Örn: Elbiseden fiyonku kaldır, Aksesuar ekleme, Yakayı düzelt..."
                className="w-full bg-white border border-gray-200 rounded-lg p-2 text-gray-900 text-xs placeholder-slate-500 resize-none focus:outline-none focus:border-mode-accent/50"
                rows={2}
              />
            ...
            {/* Purpose Selection */}
            <div className="bg-white/5 rounded-lg p-2 border border-white/10">
              <h3 className="text-gray-900 text-xs font-medium mb-2">Amaç</h3>
              <div className="space-y-1.5">
                {WOMENS_FASHION_PURPOSE_OPTIONS.map((purpose) => (
                  <button
                    key={purpose.id}
                    onClick={() => updateSetting('purpose', purpose.id as WomensFashionSettings['purpose'])}
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <span className="text-gray-900 text-xs font-medium block">{purpose.name}</span>
            ...
            {/* Pose Style */}
            <div className="bg-white/5 rounded-lg p-2 border border-white/10">
              <h3 className="text-gray-900 text-xs font-medium mb-2">Poz Stili</h3>
              <div className="relative">
                <select
                  value={settings.poseStyle}
                  onChange={(e) => updateSetting('poseStyle', e.target.value)}
                  className="w-full py-2 px-3 pr-8 rounded-lg bg-white border border-gray-200 text-gray-900 text-xs appearance-none cursor-pointer hover:bg-gray-100 focus:outline-none focus:border-mode-accent"
                >
                  {WOMENS_FASHION_POSE_OPTIONS.map((pose) => (
              ...
              <p className="text-[10px] text-gray-500 mt-1">
                {WOMENS_FASHION_POSE_OPTIONS.find(p => p.id === settings.poseStyle)?.description}
              </p>
            </div>

            {/* Background Style */}
            <div className="bg-white/5 rounded-lg p-2 border border-white/10">
              <h3 className="text-gray-900 text-xs font-medium mb-2">Arka Plan Stili</h3>
              <div className="grid grid-cols-5 gap-1.5">
                {WOMENS_FASHION_BACKGROUND_STYLES.map((style) => (
                  <button
                    key={style.id}
                    onClick={() => updateSetting('backgroundStyle', style.id)}
                    title={`${style.name} - ${style.description}`}
                    className={`w-full aspect-square rounded-md border-2 transition-all flex items-center justify-center text-lg ${
                      settings.backgroundStyle === style.id
                        ? 'border-mode-accent bg-mode-accent/20 scale-105'
                        : 'border-gray-200 bg-white hover:border-white/30 hover:bg-gray-100'
                    }`}
                  >
                    {style.icon}
                  </button>
              ...
              <p className="text-[10px] text-gray-500 mt-1.5">
                {WOMENS_FASHION_BACKGROUND_STYLES.find(s => s.id === settings.backgroundStyle)?.name} - {WOMENS_FASHION_BACKGROUND_STYLES.find(s => s.id === settings.backgroundStyle)?.description}
              </p>
            </div>

            {/* Background Color - Only show when style supports color */}
            {WOMENS_FASHION_BACKGROUND_STYLES.find(s => s.id === settings.backgroundStyle)?.supportsColor && (
              <div className="bg-white rounded-xl p-2 border border-gray-200">
                <h3 className="text-gray-900 text-[10px] font-medium mb-1.5">Arka Plan Rengi</h3>
                <div className="grid grid-cols-10 gap-1">
                  {WOMENS_FASHION_BACKGROUNDS.map((bg) => (
                    <button
                      key={bg.id}
                      onClick={() => updateSetting('background', bg.id)}
                      title={`${bg.name} - ${bg.description}`}
                      className={`w-full aspect-square rounded border-2 transition-all ${
                        settings.background === bg.id ? 'border-white scale-110 ring-1 ring-mode-accent' : 'border-transparent hover:border-white/50'
                      }`}
                    ...
                    >
                      {settings.background === bg.id && (
                        <Check
                          ...
                          style={{ color: ['white', 'soft-pink', 'blush', 'cream', 'beige', 'lavender', 'sage'].includes(bg.id) ? '#333' : '#fff' }}
                        />
                      )}
                    </button>
                  ))}
            ...
            {settings.backgroundStyle === 'studio-minimal' && (
              <div className="bg-white/5 rounded-lg p-2 border border-white/10">
                <h3 className="text-gray-900 text-xs font-medium mb-2">Studio Minimal - Renk Tonu ({STUDIO_MINIMAL_GRADIENT_COLORS.length} renk)</h3>
                    ...
                    {STUDIO_MINIMAL_GRADIENT_COLORS.map((color) => {
                      ...
                      return (
                        <button
                          ...
                          title={color.name}
                          className={`w-full aspect-square rounded border-2 transition-all ${
                            settings.studioMinimalColor === color.id ? 'border-white scale-110 ring-1 ring-mode-accent z-10' : 'border-transparent hover:border-white/50 hover:scale-105'
                          }`}
                ...
                <p className="text-[10px] text-gray-500 mt-1.5">
                  Seçili: <span className="text-mode-accent">{STUDIO_MINIMAL_GRADIENT_COLORS.find(c => c.id === settings.studioMinimalColor)?.name}</span>
                </p>
                ...
                <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-200">
                  ...
                  <button
                    ...
                    className={`w-10 h-5 rounded-full transition-all relative ${
                      settings.studioMinimalVignette ? 'bg-mode-accent' : 'bg-gray-600'
                    }`}
                  >
            ...
            {settings.backgroundStyle === 'wood' && (
              <div className="bg-white/5 rounded-lg p-2 border border-white/10">
                <h3 className="text-gray-900 text-xs font-medium mb-2">Ahşap Tipi</h3>
                <div className="grid grid-cols-4 gap-1.5">
            ...
            {/* Number of Images */}
            <div className="bg-white/5 rounded-lg p-2 border border-white/10">
              <h3 className="text-gray-900 text-xs font-medium mb-2">Görsel Adedi</h3>
              <div className="grid grid-cols-4 gap-1">
                {[1, 2, 3, 4].map((num) => (
                  <button
                    key={num}
            ...
            <div className="bg-white/5 rounded-lg p-2 border border-white/10">
              <h3 className="text-gray-900 text-xs font-medium mb-2">Görsel Ölçüsü</h3>
            ...
            <div className="bg-white/5 rounded-lg p-2 border border-white/10">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <span className="text-lg">🐾</span>
                  <span className="text-gray-900 text-xs font-medium">Hayvan Ekle</span>
                </div>
                <button
                  ...
                  {settings.animal?.animalType === 'custom' ? (
                    <div>
                      ...
                      <textarea
                        ...
                        className="w-full bg-white text-gray-900 text-xs rounded-lg px-3 py-2 border border-gray-200 focus:outline-none focus:border-mode-accent resize-none h-20"
                      />
                      <p className="text-gray-500 text-[9px] mt-1">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        className="w-full bg-white text-gray-900 text-xs rounded-lg px-3 py-2 border border-gray-200 focus:outline-none focus:border-mode-accent"
                      >
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <select
                        ...
                        className="w-full bg-white text-gray-900 text-xs rounded-lg px-2 py-2 border border-gray-200 focus:outline-none focus:border-mode-accent"
                      >
            ...
            {/* Açıklama Kartı */}
            <div className="bg-gradient-to-br from-primary/20 to-primary/20 rounded-xl p-3 border border-primary/30">
              <div className="flex items-start gap-2">
                <ReferenceIcon className="w-5 h-5 text-secondary flex-shrink-0 mt-0.5" />
                <div>
                  <h3 className="text-gray-900 text-xs font-medium">Referanslı Üretim</h3>
            ...
            <div className="bg-white/5 rounded-lg p-2 border border-white/10">
              <h3 className="text-gray-900 text-xs font-medium mb-3">Görseller</h3>
                ...
                <div>
                  ...
                  {refSourceModelPreview ? (
                    <div className="relative group aspect-[3/4] rounded-lg overflow-hidden">
                      ...
                      <div className="absolute top-2 left-2 px-1.5 py-0.5 rounded bg-primary text-gray-900 text-[8px] font-medium">
                        KAYNAK
                      </div>
                    </div>
                  ) : (
                    <button
                      ...
                      className="w-full aspect-[3/4] rounded-lg border-2 border-dashed border-primary/50 hover:border-primary flex flex-col items-center justify-center text-secondary hover:text-secondary transition-all bg-primary/5 hover:bg-primary/10"
                    >
              ...
              {refSourceModelPreview && refPosePreview && (
                <div className="flex items-center justify-center mt-3 text-gray-500">
                  <div className="flex items-center gap-2 text-[10px]">
                    <span className="text-secondary">Manken</span>
                    <span>+</span>
                    <span className="text-secondary">Poz</span>
                    <span>=</span>
            ...
            {/* Koruma Seçenekleri */}
            <div className="bg-white/5 rounded-lg p-2 border border-white/10">
              <h3 className="text-gray-900 text-xs font-medium mb-2">Koruma Seçenekleri</h3>
                ...
                {[
                  ...
                  >
                    ...
                    <div className="flex-1 text-left">
                      <span className={`text-xs font-medium ${refSettings[option.key] ? 'text-secondary' : 'text-gray-600'}`}>
                        {option.label}
                      </span>
                      <p className="text-[9px] text-gray-500">{option.desc}</p>
                    </div>
                    <div className={`w-4 h-4 rounded-full flex items-center justify-center ${
                      refSettings[option.key] ? 'bg-primary' : 'bg-gray-100'
                    }`}>
            ...
            {/* Pozlar */}
            <div className="bg-white/5 rounded-lg p-2 border border-white/10">
              <h3 className="text-gray-900 text-xs font-medium mb-2">Pozlar</h3>
                  ...
                  .map((pose) => (
                    ...
                    >
                      <div className={`w-3 h-3 rounded-full flex-shrink-0 ${
                        refSettings.selectedPose === pose.id ? 'bg-primary' : 'bg-white/20'
                      }`}>
                        {refSettings.selectedPose === pose.id && <Check className="w-3 h-3 text-gray-900" />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <span className={`text-xs font-medium block ${refSettings.selectedPose === pose.id ? 'text-secondary' : 'text-gray-700'}`}>
                          {pose.label}
                        </span>
            ...
            {/* Hayvan Ekleme - Referanslı Üretim */}
            <div className="bg-white/5 rounded-lg p-2 border border-white/10">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <span className="text-base">🐾</span>
                  <h3 className="text-gray-900 text-xs font-medium">Hayvan Ekle</h3>
                </div>
                <button
                  onClick={() => updateRefAnimalSetting('enabled', !refSettings.animal?.enabled)}
                  className={`w-10 h-5 rounded-full transition-all relative ${
                    refSettings.animal?.enabled ? 'bg-primary' : 'bg-white/20'
                  }`}
                >
                  <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-all ${
                    refSettings.animal?.enabled ? 'left-5' : 'left-0.5'
                  }`} />
                </button>
              </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-1">
                      {ANIMAL_TYPES.map((animal) => (
                        ...
                        >
                          <span className="text-lg">{animal.icon}</span>
                          <p className={`text-[9px] ${refSettings.animal?.animalType === animal.id ? 'text-secondary' : 'text-gray-600'}`}>
                            {animal.name}
                          </p>
                        </button>
                      ))}
                  ...
                  {refSettings.animal?.animalType === 'custom' && (
                    <div>
                      ...
                      <textarea
                        ...
                        className="w-full h-16 px-2.5 py-2 rounded-lg bg-white border border-gray-200 text-gray-900 text-xs placeholder:text-gray-500 focus:outline-none focus:border-primary/50 resize-none"
                      />
                    </div>
                  )}

                  {/* Cins Seçimi - Sadece custom değilse göster */}
                  {refSettings.animal?.animalType !== 'custom' && getBreedOptions(refSettings.animal?.animalType || 'dog').length > 0 && (
                    <div>
                      <p className="text-[10px] text-gray-600 mb-1">Cins</p>
                      <select
                        value={refSettings.animal?.breed || ''}
                        onChange={(e) => updateRefAnimalSetting('breed', e.target.value)}
                        className="w-full px-2 py-1.5 rounded-lg bg-white border border-gray-200 text-gray-900 text-xs focus:outline-none focus:border-primary/50"
                      >
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-4 gap-1">
                      {ANIMAL_POSITIONS.slice(0, 4).map((pos) => (
                        <button
                          key={pos.id}
                          onClick={() => updateRefAnimalSetting('position', pos.id)}
                          className={`p-1 rounded-md text-center transition-all ${
                            refSettings.animal?.position === pos.id
                              ? 'bg-primary/30 border border-primary/50'
                              : 'bg-white border border-gray-200 hover:bg-gray-100'
                          }`}
                        >
                          <span className="text-xs">{pos.icon}</span>
                          <p className={`text-[8px] ${refSettings.animal?.position === pos.id ? 'text-secondary' : 'text-gray-600'}`}>
                            {pos.name}
                          </p>
                        </button>
                      ))}
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-5 gap-1">
                      {ANIMAL_LOOK_DIRECTIONS.map((dir) => (
                        <button
                          key={dir.id}
                          onClick={() => updateRefAnimalSetting('lookDirection', dir.id)}
                          className={`p-1 rounded-md text-center transition-all ${
                            refSettings.animal?.lookDirection === dir.id
                              ? 'bg-primary/30 border border-primary/50'
                              : 'bg-white border border-gray-200 hover:bg-gray-100'
                          }`}
                        >
                          <span className="text-xs">{dir.icon}</span>
                          <p className={`text-[7px] ${refSettings.animal?.lookDirection === dir.id ? 'text-secondary' : 'text-gray-600'}`}>
                            {dir.name}
                          </p>
                        </button>
                      ))}
            ...
            {/* Kadraj Seçimi */}
            <div className="bg-white/5 rounded-lg p-2 border border-white/10">
              <h3 className="text-gray-900 text-xs font-medium mb-2 flex items-center gap-2">
              ...
              <div className="space-y-1 max-h-[200px] overflow-y-auto pr-1">
                {SHOT_TYPE_OPTIONS.map((shot) => (
                  ...
                  >
                    <div className={`w-3 h-3 rounded-full flex-shrink-0 ${
                      (refSettings.shotType || 'auto') === shot.id ? 'bg-primary' : 'bg-gray-200'
                    }`}>
                      {(refSettings.shotType || 'auto') === shot.id && <Check className="w-3 h-3 text-white" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <span className={`text-xs font-medium block ${(refSettings.shotType || 'auto') === shot.id ? 'text-primary' : 'text-gray-700'}`}>
                        {shot.name}
                      </span>
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="bg-gradient-to-br from-amber-500/20 to-orange-500/20 rounded-xl p-3 border border-amber-500/30">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <h3 className="text-gray-900 text-xs font-medium">Sahneye Yerleştir</h3>
                  <p className="text-gray-600 text-[10px] mt-1">
            ...
            {/* Poz Talimatı */}
            <div className="bg-white/5 rounded-lg p-2 border border-white/10">
              <h3 className="text-gray-900 text-xs font-medium mb-2">Poz Talimatı</h3>
              ...
              <textarea
                ...
                placeholder="Örn: Kollarını kavuşturmuş, hafif yana dönük, güler yüzlü..."
                className="w-full h-16 px-2.5 py-2 rounded-lg bg-white border border-gray-200 text-gray-900 text-xs placeholder:text-gray-500 focus:outline-none focus:border-amber-500/50 resize-none"
              />
            </div>
                  ...
                  {sceneSettings.animal?.animalType !== 'custom' && getBreedOptions(sceneSettings.animal?.animalType || 'dog').length > 0 && (
                    <div>
                      ...
                      <select
                        ...
                        className="w-full px-2 py-1.5 rounded-lg bg-white border border-gray-200 text-gray-900 text-xs focus:outline-none focus:border-amber-500/50"
                      >
            ...
            <div className="bg-gradient-to-br from-primary/20 to-primary/20 rounded-xl p-3 border border-primary/30">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <h3 className="text-gray-900 text-xs font-medium">Hazır Sahneler</h3>
                  <p className="text-gray-600 text-[10px] mt-1">
                    Kıyafet yükleyin, model sıfırdan oluşturulacak ve hazır sahneye yerleştirilecek. Doğal ve profesyonel sonuç.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white/5 rounded-lg p-2 border border-white/10">
              <h3 className="text-gray-900 text-xs font-medium mb-2 flex items-center gap-2">
                <DressIcon className="w-3.5 h-3.5 text-secondary" />
                Kıyafetler
              </h3>
              <div className="flex gap-2 flex-wrap">
                {presetClothesPreviews.map((preview, index) => (
                  <div key={index} className="relative group w-16 h-16 flex-shrink-0">
                    <img src={preview} alt={`Kıyafet ${index + 1}`} className="w-full h-full object-cover rounded-lg ring-1 ring-primary/50" />
                    <button
              ...
              <p className="text-gray-500 text-[9px] mt-2">Max 7 kıyafet (elbise, bluz, etek vb.)</p>
            </div>
            ...
            <div className="bg-white/5 rounded-lg p-2 border border-white/10">
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-gray-900 text-xs font-medium flex items-center gap-2">
                  <User className="w-3.5 h-3.5 text-mode-accent" />
                  Kendi Mankenim
                </h3>
                <button
                  onClick={() => setPresetUseCustomModel(!presetUseCustomModel)}
                  className={`w-10 h-5 rounded-full transition-all relative ${
                    presetUseCustomModel ? 'bg-mode-accent' : 'bg-gray-600'
                  }`}
                >
                  <div className={`w-4 h-4 bg-white rounded-full absolute top-0.5 transition-all ${
                ...
                <div className="flex gap-2">
                  {presetModelPreview ? (
                    <div className="relative group w-20 h-24 flex-shrink-0">
                      <img src={presetModelPreview} alt="Manken" className="w-full h-full object-cover rounded-lg ring-2 ring-mode-accent" />
                      <button
                    ...
                    <button
                      ...
                      className="w-20 h-24 rounded-lg border-2 border-dashed border-mode-accent/50 hover:border-mode-accent flex flex-col items-center justify-center text-mode-accent hover:text-mode-accent transition-all"
                    >
                  ...
                  <div className="flex-1">
                    <p className="text-gray-600 text-[9px]">Kendi manken fotoğrafınızı yükleyin. Model aynen kopyalanıp kıyafetler üzerine giydirilecek.</p>
                  </div>
                </div>
              ) : (
                <p className="text-gray-500 text-[9px]">Kapatıldığında AI otomatik model oluşturur. Açtığınızda kendi mankeninizi kullanabilirsiniz.</p>
              )}
            </div>
            ...
            <div className="bg-white/5 rounded-lg p-2 border border-white/10 space-y-3">
              {/* Görsel Oranı */}
              <div>
                <h3 className="text-gray-900 text-xs font-medium mb-2">Görsel Oranı</h3>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <h3 className="text-gray-900 text-xs font-medium">Kamera Açısı</h3>
                  <div className="flex items-center gap-2">
            ...
            {/* Görsel Sayısı */}
            <div className="bg-white/5 rounded-lg p-2 border border-white/10">
              <h3 className="text-gray-900 text-xs font-medium mb-2">Görsel Sayısı</h3>
          ...
          {productionMode === 'standard' ? (
            <>
              ...
              {isProcessing && progress && (
                <div className="bg-mode-accent/10 border border-mode-accent/30 rounded-lg p-2">
              ...
              <button
                ...
                className={`w-full py-3 rounded-xl font-medium text-gray-900 text-sm flex items-center justify-center gap-2 ${
                  !refSourceModel || !refPoseImage || isProcessing
                    ? 'bg-gray-600 cursor-not-allowed'
                    : 'bg-gradient-to-r from-primary to-primary hover:from-primary hover:to-primary/90'
                }`}
              >
                {isProcessing ? (
                  <>
            ...
            <>
              ...
              {progress && (
                <div className="bg-primary/10 rounded-lg p-2 border border-primary/20">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-secondary text-[10px]">{progress}</span>
                  </div>
                  {progressCount && (
                    <div className="mt-1.5 h-1 bg-primary/50 rounded-full overflow-hidden">
                      <div
              ...
              {error && (
                <div className="bg-red-500/10 rounded-lg p-2 border border-red-500/20 flex items-start gap-2">
        ...
        {productionMode === 'standard' ? (
            <div className="bg-white p-6 flex-1 flex flex-col min-h-0">
              {results.length > 0 ? (
                <div className="space-y-3">
                  <div className="relative group">
                    ...
                    <div className="absolute bottom-2 left-2 px-2 py-1 rounded-md bg-gray-500/50 text-gray-900 text-[10px]">
                      {WOMENS_FASHION_PURPOSE_OPTIONS.find(p => p.id === results[selectedResult]?.purpose)?.name || 'Kadın Giyim'}
                    </div>
                  </div>
                      ...
                      {results.map((result, index) => (
                        <button
                          ...
                          className={`w-16 h-16 rounded-lg overflow-hidden border-2 ${
                            selectedResult === index ? 'border-mode-accent' : 'border-transparent hover:border-white/50'
                          }`}
                        >
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-gray-500">
                  <DressIcon className="w-16 h-16 mb-4 opacity-30" />
                  <p className="text-sm text-center">Ürün yükleyip ayarları seçin</p>
                </div>
              )}
            </div>
        ) : productionMode === 'referenced' ? (
            <div className="bg-white p-6 flex-1 flex flex-col min-h-0">
              {refResults.length > 0 ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-gray-500">
                  <ReferenceIcon className="w-16 h-16 mb-4 opacity-30" />
                  <p className="text-sm text-center mb-2">Manken ve referans fotoğrafı yükleyin</p>
              ...
              {ref2Results.length > 0 ? (
                <div className="space-y-3">
                  <div className="relative group">
                    ...
                    <div className="absolute bottom-2 left-2 px-2 py-1 rounded-md bg-indigo-500/80 text-white text-[10px]">
                      Referanslı 2 Üretim
                    </div>
                  </div>
                    ...
                    <div className="flex gap-2">
                      {ref2Results.map((result, index) => (
                        <button
                          ...
                          className={`w-16 h-16 rounded-lg overflow-hidden border-2 ${
                            ref2SelectedResult === index ? 'border-indigo-500' : 'border-transparent hover:border-white/50'
                          }`}
                        >
                          <img src={result.imageData} alt={`${index + 1}`} className="w-full h-full object-cover" />
                        </button>
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-gray-500">
                  ...
                  <p className="text-sm text-center mb-2">Manken, kıyafet ve mekan fotoğrafı yükleyin</p>
                  <div className="flex items-center gap-4 text-xs text-gray-600">
                    <div className="flex items-center gap-1">
            ...
            <div className="bg-white p-6 flex-1 flex flex-col min-h-0">
              {sceneResults.length > 0 ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-gray-500">
                  <SceneIcon className="w-16 h-16 mb-4 opacity-30" />
                  <p className="text-sm text-center mb-2">Manken ve sahne fotoğrafı yükleyin</p>
                  <div className="flex items-center gap-4 text-xs text-gray-600">
                    <div className="flex items-center gap-1">
            ...
            <div className="bg-white p-6 flex-1 flex flex-col min-h-0">
              {presetResults.length > 0 ? (
                    ...
                    <div className="flex gap-2">
                      {presetResults.map((result, index) => (
                        <button
                          ...
                          className={`w-16 h-16 rounded-lg overflow-hidden border-2 ${
                            presetSelectedResult === index ? 'border-primary' : 'border-transparent hover:border-white/50'
                          }`}
                        >
                          <img src={result.imageData} alt={`${index + 1}`} className="w-full h-full object-cover" />
                        </button>
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-gray-500">
                  ...
                  <p className="text-xs text-center mb-2">Kıyafetler yükleyin ve hazır sahne seçin</p>
                  <div className="flex items-center gap-4 text-[10px] text-gray-600">
                    ...
                    <span>+</span>
                    <div className="flex items-center gap-1">
                      <div className="w-3 h-3 rounded bg-mode-accent/50" />
                      <span>Manken (Opsiyonel)</span>
                    </div>
                    <span>+</span>
                    <div className="flex items-center gap-1">
...
export default WomensFashionMode;
